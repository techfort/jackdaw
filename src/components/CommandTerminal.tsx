...
export const CommandTerminal: React.FC = () => {
  ...
  const helpLines = [
    ...
    'record              toggle recording (needs an armed track)',
    '── Navigation ────────────────────────────────────────',
    'go <time|start|end> jump playhead (seconds, mm:ss, bar.beat, start/end)',
    ...
    'rn <ref> "name"     rename a track',
    'arm <ref>           toggle record-arm on a track',
    'm <ref>             toggle mute on track',
    's <ref>             toggle solo on track',
    'vu / volup [r] [n]  raise track volume (default 0.1)',
    'vd / voldown [r][n] lower track volume (default 0.1)',
    'freeze <ref>        freeze track (owner only)',
    'unfreeze <ref>      unfreeze track (owner only)',
    '── Session ───────────────────────────────────────────',
    'save                save the current song now',
    'import              open file picker to import stems',
    'tempo <bpm>         set tempo (20–300)',
    'tempo sheet         toggle the variable-tempo sheet panel',
    'marker <1|2> [t]    set or clear a marker (t = time)',
    'marker <1|2> label "text"   rename a marker',
    'undo                undo last action',
    'redo                redo last undone action',
    'tool <select|scissors|mute> switch the active editing tool',
    'snap                toggle snap-to-grid',
    'follow              toggle follow-playhead',
    'mixer               toggle the mixer panel',
    'timeline <time|beats>      switch timeline ruler mode',
    ...
    'resolve <id>        toggle a comment resolved/open',
    ...
    '── View / Tools ──────────────────────────────────────',
    'spectrum / spec     toggle spectrum analyser panel',
